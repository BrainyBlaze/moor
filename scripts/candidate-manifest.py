...
and the one-line-per-target SHA256SUMS in target-table order.
...
# closure per target. Lane names are the canonical labels the hosted lanes in
# .github/workflows/release-candidate.yml and native-hosted.yml report, so a
# record cannot cite a lane the frozen matrix does not assign to that
# target/gate.
#
# Linux compatibility spans Ubuntu and Alpine on the exact bytes; macOS has one
# native lane per architecture.
REQUIRED = {
    "x86_64-unknown-linux-musl": {
...
# Lanes the frozen matrix assigns but that a candidate does not have to carry.
# The set is empty: every lane of the matrix runs on a GitHub-hosted runner, so
# a candidate always states "full-matrix" and never carries an `unverified`
# list. The mechanism stays because the manifest's `coverage` object is part
# of the consumer pin contract (docs/release-manifest-v1.md) and because a
# future lane that needs enrolment can be deferred here without changing the
# consumer; scripts/candidate-manifest-test.py exercises the other labels
# against a synthetic deferred set.
DEFERRED = {}
