...
cargo fmt --all -- --check
files=$(find . \( -path './.git' -o -path './target' -o -path './tests' -o -path './docs' -o -path './spec' \) -prune -o -type f \( -name '*.rs' -o -name '*.c' -o -name '*.h' -o -name '*.cpp' -o -name '*.m' -o -name '*.mm' -o -name '*.sh' -o -name '*.py' \) ! -path './scripts/count-production-loc.sh' -print)
count=0
