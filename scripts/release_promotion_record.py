...
def validate_qa_projection(value, expected_repository):
    """Validate only the release-QA fields projected into promotion records.

    The producing ``release-qa-record.py`` remains authoritative for the full
    QA schema.  This validator deliberately repeats only the identities and
    byte digests that the promotion contract consumes.
    """
    ...
    if value["repository"] != f"https://github.com/{expected_repository}":
    ...
    if not isinstance(manual_qa, dict) or manual_qa.get("verdict") != "passed":
...
def validate_expected_assets(value, qa):
    ...
    for index, entry in enumerate(value):
        ...
        positive_int(entry["size"], f"expected asset {index} size")
...
def validate_manifest_inputs(args):
    ...
    qa_value, _ = read_json(
        args.qa_record,
        "release QA record",
        canonical_encoder=legacy_canonical_json,
    )
...
def validate_manifest(value, body=None, expected=None):
    ...
    safe_name(candidate_qa["artifactName"], "manifest candidate-QA artifact name")
...
def validate_expected_asset_shape(value, what):
    ...
    for index, entry in enumerate(value):
        ...
        positive_int(entry["size"], f"{what} asset {index} size")
...
def validate_published_assets(value, manifest_assets):
    ...
    for index, entry in enumerate(value):
        ...
        positive_int(entry["size"], f"published asset {index} size")
...
def validate_completion(value, body=None, expected=None):
    ...
    for index, entry in enumerate(assets):
        ...
        positive_int(entry["size"], f"completion asset {index} size")
...
def loose_record(body, marker):
    ...
    if not body.startswith(marker_text):
        return None
    try:
        value = json.loads(body[len(marker_text) :])
...
def _existing_output_is_safe(path):
    try:
        status = os.lstat(path)
    except FileNotFoundError:
        return None
    except OSError as error:
        reject(f"cannot inspect evidence output: {error}")
    if stat.S_ISLNK(status.st_mode):
        reject("transaction evidence output is a symlink")
    if not stat.S_ISREG(status.st_mode):
        reject("transaction evidence output is not a regular file")
    return status
...
def build_evidence_manifest(transaction_root, output_path):
    try:
        root_status = os.lstat(transaction_root)
    except OSError as error:
        reject(f"cannot inspect transaction root: {error}")
    if stat.S_ISLNK(root_status.st_mode):
        reject("transaction root is a symlink")
    if not stat.S_ISDIR(root_status.st_mode):
        reject("transaction root is not a directory")

    root_absolute = os.path.abspath(transaction_root)
    output_absolute = os.path.abspath(output_path)
    output_status = _existing_output_is_safe(output_absolute)
    output_relative = None
    try:
        if os.path.commonpath([root_absolute, output_absolute]) == root_absolute:
            output_relative = os.path.relpath(output_absolute, root_absolute).replace(
                os.sep, "/"
            )
            validate_evidence_path(output_relative, "transaction evidence output path")
    except ValueError:
        output_relative = None
    entries = []
    ...
    try:
        ...
        reject(f"cannot open transaction root without following links: {error}")
    opened_root_status = os.fstat(root_fd)
    if not _same_file(root_status, opened_root_status):
        os.close(root_fd)
        reject("transaction root changed while being opened")
    ...
    def inventory(directory_fd, relative_components):
        ...
        for child in children:
            ...
            if relative == output_relative:
                if output_status is not None and not _same_file(
            ...
            if len(body) != file_status.st_size:
...
def write_evidence_output(path, body):
    ...
    safe_name(name, "transaction evidence output name")
    parent_fd = _open_absolute_directory_no_links(
        parent, "transaction evidence output"
    )
    descriptor = None
    try:
        ...
        try:
            ...
            try:
                ...
                reject(f"cannot inspect transaction evidence output: {error}")
            if stat.S_ISLNK(expected.st_mode):
                reject("transaction evidence output is a symlink")
            if not stat.S_ISREG(expected.st_mode):
                reject("transaction evidence output is not a regular file")
            descriptor, observed = _open_no_follow_at(
                parent_fd,
                name,
                os.O_WRONLY,
                expected,
            ...
            if not stat.S_ISREG(observed.st_mode):
                reject("transaction evidence output is no longer a regular file")
        ...
        with os.fdopen(descriptor, "wb", closefd=True) as handle:
            ...
            if not stat.S_ISREG(current.st_mode) or not _same_file(
                ...
                reject("transaction evidence output changed while being written")
    except OSError as error:
...
def run_command(args):
    ...
    if args.record == "evidence":
        value = build_evidence_manifest(args.transaction_root, args.out)
        body = canonical_json(value)
        write_evidence_output(args.out, body)
        print(f"wrote {args.out} ({len(body)} bytes)")
        return
