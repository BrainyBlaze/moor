...
#[test]
fn discovery_suffixes_are_exact_and_case_sensitive() {
    // POSIX names are bytes: `.exit` is a discovery artifact, `.EXIT` is a
    // session name — there is no case-folding filesystem to accommodate.
    for suffix in [".log", ".events", ".instrument"] {
        assert_eq!(session_name(format!("session{suffix}").into()), None);
    }
    assert_eq!(
        session_name(OsString::from("session.exit")),
        Some(OsString::from("session"))
    );
    // `.EXIT` is not a discovery suffix, so the whole name IS the session.
    assert_eq!(
        session_name(OsString::from("session.EXIT")),
        Some(OsString::from("session.EXIT"))
    );
    assert_eq!(
        session_name(OsString::from("session.LOG")),
