...
// terminated and released the `Store` it owns — a caller that removes the
// directory right after the drop must never race a worker still holding a
// handle into it. The observable guarantee is that `Lane::drop` JOINS the
// worker before returning. We capture the
...
fn dropping_a_lane_joins_its_worker_before_returning() {
    ...
    // And with the store handle provably released, the directory delete
    // succeeds outright.
