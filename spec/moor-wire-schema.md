...
The supervision source writes exactly one 32-byte record to a private one-way channel and closes its write end. The actual holder consumes that channel through `<BASENAME>_LAUNCH_CHANNEL` and requires EOF after byte 32. This record is a freshness discriminator, not authorisation; same-user trust remains §11.1 of the specification.

On Windows background `start`, the first Moor process is only a transport bridge. It accepts exactly one of a canonical native `<BASENAME>_LAUNCH_CHANNEL` source handle or invocation-derived `<BASENAME>_LAUNCH_ADAPTER=stdin-v1` with a valid byte-stream standard-input handle. Adapter plus native selector, another adapter value, another command mode, missing/invalid/non-pipe stdin, or an invalid native selector fails closed. Lookup and removal are case-insensitive. The bridge privately duplicates the source, removes both source selectors, replaces process standard input with guarded NUL, gives the detached process NUL stdin, and forwards the private source with `windows-spawn`'s explicit handle table. The selector inherited by the detached holder names the child-table duplicate created by `windows-spawn`, **not** the creator's raw handle value. The first process never reads or decodes V21.
...
The whole read including EOF has a 2-second deadline. A selector that is present but malformed, a handle outside the explicit inheritance list, a non-byte-stream handle, a short or long record, timeout, bad magic/format/reserved field, zero or mismatched generation is a failed supervised launch and never falls back to unsupervised. Only the detached holder's complete valid V21 plus EOF and matching carriers establishes supervision; the adapter, selector, preparation envelope, or carrier alone never does. The launch selector/read handle, adapter, preparation/receipt selectors, and every transferred preparation capability are removed or closed before the requested child is created, and none may appear in that child's explicit inheritance table.
...
No flag, padding, trailing diagnostic, or alternate length exists. Every record write is checked for all 12 bytes. A short write or write error poisons and closes the reporter, and no destructor may append state 3 afterward.

`store-adopted` is sent only after the holder owns the lifecycle/event/log writer leases and has captured every created-object identity and created-directory capability/fact, but before initial commits, requested-child launch, or rendezvous publication. For the prepared Windows route, the holder continues only after the creator consumes that complete record, irrevocably drops its rollback guards/capabilities, clears the launch Job's kill-on-close limit, and sends the separate private `MOORACK1` receipt plus EOF within two seconds. That nonce-bound 32-byte receipt is creator/holder transport defined by the main specification; it is **not** a V31 record and changes no byte in this section. An incomplete state-1 delivery, missing/bad receipt, or failure to clear the Job never grants creator deletion authority by inference.

`ready` follows only after all initial commits, the child-launch gate, exact rendezvous publication, and preparation-reservation deletion. `failed` is permitted only before `ready` and reports a known failure. After process creation, a creator may roll back only on a checked never-resumed abort with exact terminate-and-wait proof or when a complete valid state-3 record is the first MORR record and exact holder death is confirmed. Zero-byte EOF, timeout, a partial/invalid/read-error stream, death without that complete first record, or any resume uncertainty is indeterminate and authorizes no deletion. Loss after adoption is resolved by authenticated identity probe and otherwise remains indeterminate; authority never transfers back by guess. Foreground `run` crosses the same states internally without this stream.
...
**V31** — all private background-result states, each exactly 12 bytes. Generation is 7; store-adopted and ready use result zero and are the two-record success sequence, with the private non-V31 receipt boundary between their semantics. Each 12-byte write must complete or poison the stream. Failed uses frozen sample code `0x1234` as an independent first-record alternative and never follows ready:
...
**Private launch channels:** supervised allocation starting at 2 and a discriminator carrying generation 1 refused; missing discriminator with inherited generation variables producing an unsupervised holder with those variables stripped; every selector grammar boundary; exact V21/V22 records; each short prefix, one extra byte, missing EOF, duplicated inherited writer and deadline; wrong magic/format/flags/reserved/generation; instrumentation wrong/zero PID or nonce; selector handles outside the explicit inheritance list; launch selector absent from the requested child; instrumentation selector/nonce present only for that child and removed before its first application instruction; POSIX constructor ACK, descendant preload with no ACK variables, Linux `LD_PRELOAD` colon/dollar/whitespace path refusal, macOS `DYLD_INSERT_LIBRARIES` colon path refusal, and Windows missing export/nonzero initializer/wrong-architecture failure.

The native-Windows subset covers native-selector forwarding and exact `stdin-v1` adapter forwarding for ordinary and renamed binaries; adapter/native mutual exclusion; bad adapter value, wrong mode, null/invalid/non-pipe stdin, noncanonical native handle, and case-insensitive lookup/removal; private duplication, installed-NUL lifetime and failure to clear it safely; detached NUL stdin; creator-source and complete suspended-command destruction; and proof that the detached selector names the `windows-spawn` child-table duplicate rather than the creator's raw handle. It covers `prepared-v1`, capability-free `failed-v1`, and no-envelope/no-holder `PreparationIndeterminate` at every mutation failpoint; exact reservation bytes and file/parent durability; crashes during reservation creation/write/flush and after every prepared object; refusal of every same-/later-generation retry while any reservation prefix exists; preserved preexisting directories and substituted path successors; blank slots never reaching ordinary store classification; and authenticated absolute event-target identity after a relative leaf is resolved.

Prepared-handle vectors require every owning raw handle distinct except the event store's declared non-owning directory borrow; same-object handles confined to one declared reservation/stage/directory/slot capability group; all cross-group aliases refused; read-attributes/share-all creator guards distinct from access-reduced holder writers; a directory DELETE capability iff the attempt created that exact directory; and requested-child exclusion of every launch, adoption, receipt, preparation, writer, publication, and deletion handle/selector. Atomic launch-guard Job assignment is proved at process creation. A vendor-crate helper covers hard creator death after successful `CreateProcessW` but before return, while real-Moor tests cover death before the vendor call, immediately after the infallible armed-wrapper construction, and every later suspended/running pre-receipt checkpoint. Tests inspect capability closure, require the complete command gone before resume, distinguish checked never-resumed abort plus exact terminate/wait from `ResumeThread` error, unexpected suspend count, and uncertain wait, and never accept ordinary destructor behavior as rollback proof.

V31 covers every MORR state, wrong magic/format/state, nonzero success result, zero generation, short/long record, checked short/error write poisoning with no destructor-appended failure, failed as the first record, ready before adoption, and state-1-before-private-receipt-before-ready ordering. The exact `MOORACK1` record covers every 0..31-byte prefix, byte 33, missing EOF, wrong magic/format/reserved/generation/nonce, duplicate, read/write error, and the shared two-second deadline. Complete first state 3 plus exact death is the only post-create report-stream rollback proof; zero-byte EOF, timeout, partial/invalid/read-error MORR, death without state 3, and every incomplete state-1 observation preserve fenced debris and delete nothing. Loss after completed state 1 never restores creator deletion authority. Every failure leaves no published rendezvous or running requested child, and no private channel substitutes for another.
