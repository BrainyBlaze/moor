...
**Companion paths are one frozen native-path mapping.** Let `R` be the resolved native rendezvous path, preserving its parent and final component. The log directory is obtained by appending `.log` to the final native path component of `R`, not by joining a child named `.log`; the lifecycle directory is obtained by appending `.exit` to the final native path component the same way. For example, `/a/name` maps to `/a/name.log` and `/a/name.exit`, never `/a/name/.log` or `/a/name/.exit`. POSIX event operands and every absolute native event operand remain exact. Windows additionally accepts §8.1's one-component leaf and resolves it beneath the exact invoked-binary Moor root; the resolved absolute target, never the leaf alone, is used by every durable and security surface. Moor never appends `.events` or derives an event target from `R`, and `.events` remains reserved against session-name collision. Instrumentation keeps §4.7's mapping: the owner-only root contains the immutable stage whose final component is exactly `<H>.instrument`, where that section defines `H`.

The supervised Windows preparation reservation is the canonical session companion `companion(R, ".exit.instrument")`. Its final component already ends in OB-1's reserved `.instrument` suffix, while the embedded non-hex `.exit` segment makes it disjoint from every immutable instrumentation stage, whose prefix is exactly 64 lowercase hexadecimal characters. This reservation therefore needs no new discovery or suffix rule and cannot be named by a legal session. Its transactional ownership and deletion rules are §11.6.

No operation on one session may unlink, truncate, clear, or otherwise disturb another session's rendezvous, store, or stage. Every destructive operation addresses the target relative to an already verified parent/container, does not follow a link or reparse point, and re-verifies the target's platform object identity immediately before mutation; it never acts on a name alone. The rendezvous has its independent stale-object fence. Outside the creator's identity-recorded prepublication rollback in §11.6, removing any companion additionally requires valid owning lifecycle evidence; missing or disagreeing evidence makes every companion unowned and non-removable.
...
- `list [-a]` — enumerate sessions in the per-user root (§2.2) only; sessions addressed by path are not listed. Discovery is a **union of two independent sources** (§4.5): the addressable rendezvous objects present in the root (POSIX sockets or Windows markers), and the durable exit records — each source enumerated on its own, because either artefact may exist with or without the other. The two are then merged by name and classified by the cross-product below, which also fixes what each of `list`, `list -a` and `rm` does with every combination. An empty result prints exactly `(no sessions)` and exits **0**.
  ...
  **[NEW] The full cross-product.** A rendezvous object may be present or absent; an exit record may be present or absent. All four combinations occur — a holder that crashed after writing its record but before unlinking its socket or marker, a cleanup interrupted midway, a new session created over an old record. In this table, "exit record" means a lifecycle companion whose selected commit and manifest independently pass §11.6's ownership checks for the requested session. A directory merely occupying the `.exit` path, or a manifest that disagrees, is not an exit record and grants no removal authority.
...
Each ancestry entry is the **absolute addressable rendezvous path** of the session, not the bare name: the socket path on Linux and macOS, the marker path on Windows. Entries are ordered outermost-first.
...
**OB-6 chose D for the original amendment — the versioned second carrier with a dual-carrier migration.** C could not close the actual contract: the ancestry contains absolute paths, so a Windows drive designator contains a colon even when the caller's session component does not, and a POSIX parent path or invoked base name may contain one too. Restricting only the session operand therefore leaves the carrier ambiguous while claiming it is fixed. D was an additive migration for existing consumers and an OB-24 correction for `current` on affected values. OB-1 remains a separate reserved-suffix naming rule; it no longer pretends to solve ancestry encoding.
...
#### 4.4.3 Creator-only launch transport

The invocation-derived `<BASENAME>_LAUNCH_ADAPTER` and every invocation-derived preparation, adoption-control, writer, publication, rollback, and Job selector of §10.1.1 are creator/holder transport only. They are removed case-insensitively from the Windows environment before detachment and are never inherited by the requested child. In particular, the adapter does not become child configuration merely because its source begins as the creator's standard input: the source is privately duplicated, forwarded through the explicit holder handle table, and the creator's source owner is destroyed before the holder can run. The child receives only the closed environment set named at the start of §4.4.
...
- **Immutable staging is mandatory.** From the validated handle the creator copies the exact bytes into an exclusively created file in the enforced owner-only per-user root. Its basename is `<H>.instrument`, where `H` is 64 lowercase hexadecimal digits encoding SHA-256 over, in order, the wide-length-prefixed canonical session identity, little-endian wire generation, and holder incarnation. The creator flushes the bytes, sets POSIX mode `0500` or the Windows protected read/execute DACL, closes the write handle, reopens the stage without following links, and verifies both its recorded file identity and SHA-256. Only this immutable stage path is placed in a loader variable or passed to Windows insertion.
- **The POSIX loader encoding is exact for the stage path.** Linux prepends the immutable stage path to `LD_PRELOAD`, separated from an inherited nonempty value by one ASCII space. macOS prepends it to `DYLD_INSERT_LIBRARIES`, separated from an inherited nonempty value by one colon. The generated stage spelling contains neither platform delimiter nor loader expansion syntax. The resulting loader variable is inherited normally, so ordinary dynamically linked descendants generally load the same staged bytes; set-id, static, loader-scrubbed, or explicitly replaced environments may not. The acknowledgement below proves only the requested initial child. On Windows the guarantee ends at that child and descendants are not automatically injected. A caller needing descendant coverage requires a separately specified producer/launcher contract and MUST NOT infer it from `-S`.
...
On POSIX a command containing `/` is executed exactly; a command without `/` uses inherited `PATH` with the platform's normal `execvp` search semantics, and `argv[0]` is the command operand exactly as supplied. Windows uses its documented executable-search order for a bare executable and this exact inverse-`CommandLineToArgvW` encoding. Any supplied program or later argument containing NUL is rejected. The command line begins with the supplied program surrounded by ASCII double quotes. An empty later argument or one containing ASCII space, tab, quote, or `|` is wrapped in ASCII double quotes. While encoding a wrapped argument, for each run of `N` backslashes: before a literal quote emit `2N+1` backslashes and then the quote; before the next ordinary UTF-16 code unit emit the `N` backslashes unchanged; before the closing quote emit `2N` backslashes. All other UTF-16 code units are emitted unchanged. A later argument with none of those quote-triggering ASCII code units is emitted unchanged. Each encoded later argument is preceded by one ASCII space. Vectors include empty to `""`, `a|b` to `"a|b"`, unquoted ordinary backslashes, and runs `N=0,1,2` before a literal quote and before the closing quote. The exact dependency pin `windows-spawn = "=0.1.0"` supplying this encoding at zero first-party lines is load-bearing pending native-Windows conformance; changing that pin requires the same byte vectors before release. `-d` is applied before lookup so relative executable paths resolve from the requested child directory.
...
**The detach byte inside the win32 keyboard carrier.** When §12.3's input-mode negotiation is honored, the viewer's own console delivers keyboard input as the platform's win32 keyboard carrier rather than bare bytes, so this section's detection must look inside it. The viewer recognizes exactly the canonical form — the introducer `ESC[` or the UTF-8 C1 CSI `C2 9B`, then exactly six explicit canonical decimal fields `Vk;Sc;Uc;Kd;Cs;Rc` separated by `;`, closed by `_` — and recognizes it solely to apply this section. `Vk`, `Sc`, `Uc`, and `Rc` are unsigned 16-bit values, `Cs` is unsigned 32-bit, `Kd` is `0` or `1`, and `Rc` is nonzero. A recognition candidate is bounded: at most 64 UTF-8 bytes and at most 50 ms for an incomplete candidate; a malformed, incomplete, or overlong candidate is replayed byte-exact into the ordinary raw-byte detach machine, and any other spelling, including a non-canonical field, is ordinary raw bytes. Recognition changes forwarding in exactly one way: a recognized sequence travels atomically — forwarded whole or consumed whole, never split.
...
- Windows end is `ended:"exited",code:<u32>`;
...
The stream is named by `-T <path>` (§3.5). POSIX requires a fully qualified absolute native path and uses its bytes exactly. Windows branches **absolute first**: when the native path API classifies the operand as absolute, the existing drive/UNC branch and the original spelling are used exactly; otherwise only the root-relative leaf grammar below is considered. A drive-relative value such as `C:dir`, root-relative value such as `\dir`, or any invalid relative spelling is rejected before cleanup, store mutation, child creation, or rendezvous publication using §13.3's event-target row with cause `not-absolute`.

The Windows leaf is inspected as raw UTF-16 code units before path-component normalization. Its length is `1..255` code units; every unit is ASCII alphanumeric, dot, underscore, or hyphen; the first and final units are ASCII alphanumeric; and the ordinal case-insensitive basename before the first dot is not `CON`, `PRN`, `AUX`, `NUL`, `CLOCK$`, `COM1` through `COM9`, or `LPT1` through `LPT9`. The Windows path-component API must still return exactly one `Normal` component equal to the original operand. These rules reject separators, prefix/colon syntax, spaces, trailing dots, empty/dot/dot-dot forms, alternate Unicode spellings, and normalized-away suffixes before filesystem access. A valid leaf resolves to `canonical Windows Moor root for the exact invoked binary / exact supplied leaf`.

Validation retains two different values. `operand` is the exact caller spelling rendered in every diagnostic; `target` is the absolute native path used for containment, DACL/type checks, creation, manifests, lifecycle state, status, cleanup, and object-identity fencing. No durable record contains only a relative leaf, and no diagnostic silently expands it. The target must be a strict descendant of the exact Moor root; root and components must have the required ownership/protection; links and reparse points are refused; and aliases with the rendezvous, log, lifecycle, instrumentation stage, or preparation reservation are refused. On every supported platform the accepted target names a directory containing exactly four regular, non-link slots named `body.0`, `body.1`, `commit.0`, and `commit.1`. The status descriptor's event-layout value `02` means this portable four-slot committed directory on every platform. Value `01` is the superseded POSIX single-file layout and is never emitted by an amended holder; disabled storage is `00`.

The event directory may be absent for Moor to create exclusively after confirmed-stale cleanup, or may be the exact validated empty directory object handed off by this caller. A present directory must be inside the session root, owner-only (`0700` on POSIX or §2.2's protected DACL on Windows), opened without following a symbolic link or reparse point, and contain no entry. Moor creates all four slots exclusively against that opened directory; an extra entry or pre-existing slot is refusal, never adoption. Slots use POSIX mode `0600` or the protected Windows file DACL. The lifecycle launch record retains the event directory's exact native path for cleanup.
...
**Windows creator adapter.** A supervised Windows holder accepts exactly one of two mutually exclusive launch sources: the native `<BASENAME>_LAUNCH_CHANNEL` selector above, or invocation-derived `<BASENAME>_LAUNCH_ADAPTER=stdin-v1`. The adapter is valid only for the background `start` creator path. Lookup, conflict detection, and removal of both invocation-derived keys use Windows' ASCII-case-insensitive environment semantics. Any other adapter value, an invalid mode, unusable standard input, or simultaneous adapter and native selector is `selector-invalid`. The adapter is transport only: it neither authenticates nor supervises, and it never reaches the requested child.

The creator treats `GetStdHandle(STD_INPUT_HANDLE)` as borrowed only long enough to validate and privately duplicate the byte-stream handle. It then opens Windows NUL, installs that NUL handle with `SetStdHandle`, and only after successful installation closes the original standard-input handle. `SetStdHandle` does not duplicate, so an `InstalledNullStdin` guard owns NUL. Checked cleanup first clears the process standard-input slot and then closes NUL; if clearing the slot fails, the guard deliberately retains NUL until creator-process termination rather than leaving a dangling closed handle. No restoration of the already-closed original is claimed. The detached holder receives null standard input independently; it never receives the creator's process-standard-handle slot.

The private source duplicate is forwarded through `env_handle_lower_hex`, which creates and records its own child-table duplicate in the `windows-spawn` command's explicit transfer list. Only after that fallible forwarding succeeds does the creator destroy its local source duplicate. Only after the local source is gone may generation-dependent filesystem preparation begin. A forwarding failure therefore enters no preparation path and creates no reservation. The complete spawn command owns the sole remaining source duplicate and is destroyed, with every command-owned transfer duplicate, before the suspended holder is resumed.

**Preparation selection and precedence.** Generation-dependent preparation is attempted only when a launch source is present and both generation carriers form one canonical equal tentative generation. Missing, malformed, out-of-range, or disagreeing carriers cause no generation-dependent mutation; the creator still supplies a handle-free `failed-v1` preparation envelope so the holder can preserve launch-record-then-carrier diagnostic precedence. After a valid pair, the creator constructs exactly one invocation-derived `BASE_PREPARATION=prepared-v1|failed-v1` envelope after removing every inherited preparation key. `prepared-v1` carries the complete metadata, capability inventory, and private acknowledgement channel. `failed-v1` carries no filesystem, adoption, writer, publication, deletion, or created-object capability. A controlled preparation failure may select `failed-v1` only after exact identity-fenced local rollback succeeds. If a partial preparation cannot be rolled back with proof, creator-local `PreparationIndeterminate` closes all remaining capabilities, constructs no envelope or holder, returns §13.3's exact `io-error`, and leaves the reservation/debris fenced for explicit whole-transaction cleanup.

The prepared inventory separates authority by capability. Creator identity guards are read-attributes/share-all handles. DELETE-capable directory handles exist only for directories this attempt created. Separate transfer-scoped, access-reduced holder handles carry writer or publication access and no DELETE access. The event target owns its one directory-created fact/capability; its `PreparedStore` only borrows that directory. Every owning raw `HANDLE` value is distinct, with that event-store borrow the sole permitted same-raw relationship. Distinct values may name the same kernel object only within one declared capability-facet group: reservation identity/deletion, either stage identity/publication, each directory's identity/access-reduced/deletion facets, or each slot's identity/writer facets. Every cross-group same-object relationship is rejected, including a launch, adoption, MORR, standard-stream, rendezvous, or requested-child handle naming a prepared object. Canonical fixed-length prepared metadata binds the transaction nonce, identities, created-directory facts, capability-presence map, borrow/equivalence map, and ordered handle-identity digest; recomputing metadata cannot manufacture deletion authority absent from the table.

**Suspended launch guard.** The creator makes one unnamed Job, enables `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`, retains its sole handle, and lends only a borrow to `windows-spawn` through `SpawnOptions::job`. `PROC_THREAD_ATTRIBUTE_JOB_LIST` therefore assigns the exact suspended holder atomically with `CreateProcessW`; the vendor never owns or transfers the Job. On successful return one private Moor integration function performs no fallible work or callback before moving the returned `SuspendedChild` and creator Job into a non-cloneable `ArmedSuspendedLaunch`. The holder cannot run until all creator writer/publication duplicates and the complete spawn command have been destroyed.

Consuming `ArmedSuspendedLaunch` performs the sole resume transition. Before any resume attempt, creator rollback is authorized only when the typestate proves the primary thread never left its original suspended state and a checked consuming terminate-and-wait operation confirms exact death. A `ResumeThread` error, unexpected prior suspend count, any state that may have run, or unproved terminate/wait is indeterminate and authorizes no deletion even if death is later observed. Ordinary `Child`/`SuspendedChild` drop is never rollback evidence. Abrupt creator death at or after successful `CreateProcessW` instead closes the sole Job handle and the kernel ends the atomically assigned suspended/running holder. The Job's kill-on-close limit is cleared only after the complete MORR state-1 transition in §11.6 and before its private acknowledgement.

The holder remains the sole launch-record validator and runtime authority. It first validates the unchanged §15.1 record, EOF, deadline, and generation carriers. A `failed-v1` envelope is accepted only when it carries no preparation capability and then yields the frozen `preparation-failed` result. A `prepared-v1` envelope is accepted only after every private selector and transferred handle is revalidated for exact type, DACL, absolute path, identity, capability fact, digest, borrow, and logical-equivalence group; missing, extra, repeated, aliased, path-substituted, or inconsistent input fails closed before store initialization. Transfer-only handles are closed when ownership enters holder state. Every private selector is removed before the requested child is created, whose explicit inheritance list excludes the launch/adoption/MORR channels, source duplicate, Job, preparation handles, and metadata.
...
This section is normative for **what the protocol must guarantee and what it must carry**. The exact field layouts, integer widths, byte order, frozen constants, deadlines and error codes are in the companion artefact — **[moor-wire-schema.md](./moor-wire-schema.md), version `wire-schema-4`** — which an implementer builds against directly. That file also freezes the Windows marker, portable committed-store record, and semantic-producer frames. Where the two disagree this section wins and the schema is a defect.
...
2. For stale residue, fence the rendezvous independently. On POSIX, hold the already verified parent-directory descriptor, use `fstatat(..., AT_SYMLINK_NOFOLLOW)` to require a socket and capture its device/inode identity when listener absence is positively established, then repeat that same no-follow type and identity check immediately before `unlinkat`. The socket itself is never opened, so this works on both Linux and macOS. On Windows, retain the non-reparse marker handle and its volume/file identity and revalidate that identity at the final path immediately before removal. A dead POSIX socket has no generation to compare because no authenticated exchange can complete; cleanup therefore never demands, guesses, or synthesizes a rendezvous generation.

   Independently open the exact `.exit` lifecycle path from §2.1 and validate its selected commit and manifest. Its `session` check is platform-specific but closed. On POSIX it must equal tag `01` plus the requested lexically resolved absolute socket-path bytes. On Windows it must be a canonical, correctly sized tag-`02` historical marker identity; the exact `.exit` address derived from the fully resolved marker path binds the lifecycle directory to the requested name. An exited Windows session has no marker whose vanished file id could be recomputed, and a combined row may contain a successor marker with another file id, so cleanup never requires the historical tag `02` to equal a current or nonexistent marker identity. The marker, when present, remains independently fenced by the preceding paragraph.
...
3. A supervised Windows preparation with a present launch source and a canonical equal tentative generation pair performs all remaining read-only target validation and randomness generation, then exclusively creates `companion(marker, ".exit.instrument")` as its **first generation-dependent filesystem mutation**. This is the canonical reservation for every generation of that session. Its fixed 32-byte record is ASCII `MOORPRE1`, format byte `01`, three zero bytes, the little-endian u32 generation, and the 16-byte transaction nonce. The creator checked-writes all 32 bytes, flushes the file, flushes the parent-directory metadata, and retains the exact file identity before creating any event directory, store slot, marker stage, or instrumentation stage. The final component already ends in OB-1's reserved `.instrument` suffix, so it needs no new naming or discovery rule; its non-hex `.exit` segment cannot equal the exact 64-lowercase-hex instrumentation-stage prefix. The alias map also rejects an event target that names this object.

   Any object already present at the reservation path — including an empty, short, malformed, partially created, or substituted object — is a deferred `preparation-failed` conflict and authorizes no mutation. A crash during creation/write/flush or after any later prepared object therefore leaves a fence that every same- or later-generation automatic attempt refuses. A live attempt may remove only its exact reservation identity, and only as the last action of a controlled identity-fenced rollback or, on the holder side, after exact marker publication and immediately before `ready`. Abrupt-death debris has no automatic collector: explicit cleanup must validate and remove the whole reserved transaction; deleting only the reservation is unsupported operator corruption.
4. The Windows prepared transaction creates blank four-slot event, lifecycle, and optional log stores but no valid initial commit. Every resolved rendezvous, reservation, event, log, lifecycle, marker stage, and instrumentation stage is pairwise distinct by canonical path and opened identity except for the declared facets of one logical object. Creator identity guards are read-attributes/share-all handles. Holder directory and slot writers are separate, access-reduced, transfer-scoped handles with no DELETE access. Each owning table entry has a distinct raw handle; the event store's non-owning borrow of its `EventTarget` directory is the sole same-raw exception. Distinct handles may identify one object only inside its declared reservation, stage, directory, or slot capability group; every cross-group same-object alias is refusal.

   A directory deletion capability exists if and only if this attempt created that exact directory. `EventTarget` alone owns the event-directory created fact and capability; the event store only borrows them. A caller-supplied validated-empty event directory has no creator or holder directory-deletion capability and survives every failure. Lifecycle and optional log stores carry their own created facts and capabilities. Capability presence plus the captured identity, never a path or metadata bit alone, is deletion authority. On determinate creator rollback, only after the exact holder has exited and every non-delete-sharing writer has closed, each guarded file is reopened for DELETE and revalidated; already retained directory capabilities remove only creator-created directories. There is no path-only fallback and every substituted successor is preserved.
5. Before resume, the creator destroys every transfer-scoped writer/publication original, the complete reusable spawn command, and every command-owned duplicate. The holder is then the only possible writer. It consumes and removes the private preparation selectors, validates the launch record and generation carriers, validates the complete handle/capability map and metadata digest, acquires lifecycle, event, then log writer leases, and captures every created-object identity and created-directory fact. This prepared-holder route occurs before blank lifecycle slots can reach `runtime::client::decide`; an environment assertion without the exact validated handle set cannot select it.
6. Only after the authority transfer described below does the holder initialize the stores, pass the child/bootstrap/instrumentation gates, confirm the requested child is alive, and make all initial commits durable. It publishes the exact retained marker-stage handle relative to the validated destination handle, reopens and verifies the final marker identity, deletes the exact reservation identity, and only then reports ready. POSIX and an unsupervised Windows holder preserve the existing holder-side exclusive provisioning order and reach the same initial-commit/publication boundary without the preparation envelope.

The Windows creator's rollback authority is a non-cloneable `PreAdoption` value containing only exact reservation/file identity guards and creator-created-directory deletion capabilities; it contains no store writer or publication writer. Separately held process, launch-guard Job, MORR, adoption-control, standard-stream, and installed-NUL cleanup handles grant no filesystem deletion authority. A determinate failure proved to precede successful `CreateProcessW`, or a checked consuming abort whose armed typestate proves the holder never resumed and whose terminate-and-wait proves exact death, may consume `PreAdoption` to remove only its exact objects in reverse order and the reservation last. Ordinary `Drop`, a `ResumeThread` error, an unexpected prior suspend count, a process that may have run, or uncertain termination/wait is never rollback evidence and removes nothing.

Background launch transfers authority over the private holder-to-creator result stream using the unchanged exact 12-byte record: ASCII `MORR`, format byte `01`, state byte (`01` store-adopted, `02` ready, `03` failed), little-endian u16 result code, and little-endian u32 generation. Every report write is checked for all 12 bytes. A short write or write error poisons and closes the reporter, and its destructor must not append state 3.

`store-adopted` is written only after the prepared holder owns the lifecycle/event/log writer leases and has captured every created-object identity, deletion-capability presence, and created-directory fact, but before any initial commit, requested-child launch, or publication. After the checked state-1 write the holder waits for one private 32-byte acknowledgement plus EOF: ASCII `MOORACK1`, format byte `01`, three zero bytes, the expected little-endian u32 generation, and the exact 16-byte preparation nonce. The complete acknowledgement and EOF share one monotonic two-second deadline. A 0..31-byte EOF, any 33rd byte, wrong magic/format/reserved/generation/nonce, read error, duplicate, or late receipt fails closed before store initialization or child launch.

The creator accepts only one complete valid state-1 record for the expected generation. It then consumes `PreAdoption` into `Adopted`, dropping every identity guard and directory-deletion capability without deletion, clears kill-on-close on the exact launch-guard Job, checked-writes the acknowledgement, and closes its writer. If clearing the Job limit fails it sends no acknowledgement, retains containment while the holder performs exact holder-side cleanup, and waits for exact death; creator deletion authority is not restored. After a successful clear, missing acknowledgement yields control EOF and holder-side cleanup. Only an acknowledged holder can continue to the initial-commit and publication boundary. State 2 (`ready`) remains after those commits, the child-launch gate, exact marker publication, and reservation deletion.

After successful `CreateProcessW`, creator rollback is authorized only by the checked never-resumed abort above or when a complete valid state-3 record is the **first** MORR record for the expected generation and exact holder death is then confirmed. Clean zero-byte EOF, timeout, MORR read error, an invalid record, a 1..11-byte prefix, any resume uncertainty, unconfirmed death, or death without that complete first state-3 record is indeterminate and authorizes no deletion even if death is observed later. One-way MORR cannot distinguish a pre-state-1 crash from a state-1-write/kill race. A complete state 1 irrevocably gives cleanup authority to the holder or a later authenticated identity-probed retirement path; the creator has no deletion API from `Adopted`. Foreground `run` crosses the same store-adopted, acknowledged, and ready states internally.

Every store has one portable exclusive writer lease on `commit.0`: nonblocking `flock(LOCK_EX)` on POSIX and an exclusive `LockFileEx` byte-range lock held through a non-delete-sharing handle on Windows. The holder acquires lifecycle, event, then log leases before publication and holds them until close. Offline `clear`, `rm`, and stale replacement acquire the same applicable order under one two-second total deadline, re-probe liveness after acquisition, and release in reverse order. Failure to acquire or a changed probe is indeterminate and performs no mutation. This serializes competing offline commands and holder writes without a fifth file.

The event directory may be caller-created only under §8.1's validated-empty exception; log and lifecycle directories are always attempt-created, either by the supervised Windows creator for transfer or directly by the holder on the other paths. Every store and stage is bound to the owning generation and incarnation and is removed only on confirmed retirement or rollback. A new generation never adopts a predecessor body, commit, manifest, reservation, stage, or legacy layout. Existing legacy log/exit residue is inventoried and drained before the amended reader is enabled.
...
Controller wire v4, semantic wire v1, JSON event schema v2, the four-slot event/log/lifecycle store, checksums, integer byte order, generations, correlations and provenance are portable. Native path values are not: POSIX surfaces carry raw bytes; Windows byte/JSON surfaces carry canonical WTF-8 converted losslessly from UTF-16, including unpaired surrogates. A decoder rejects overlong, non-canonical or non-round-tripping WTF-8.

### 12.2 Windows rendezvous and peer identity **[decision]**

Linux and macOS publish the Unix-domain socket itself. Windows publishes a protected marker file at the addressable path; the marker names a protected local named pipe. The marker's binary layout is frozen in wire-schema-4.

Before marker publication, the holder reads exactly 16 bytes from the operating system's cryptographic random source and encodes them as 32 lowercase hexadecimal digits. The pipe name is exactly `\\.\pipe\moor-` followed by those digits; no other generated spelling is conforming. It creates the first byte-mode listening instance with `FILE_FLAG_FIRST_PIPE_INSTANCE`. A name collision or failure to obtain that first instance fails the unpublished launch; Moor never connects clients to a pre-existing instance. After that ownership proof, the same holder may create further listening instances under the identical name, DACL and local-only/overlapped/byte-mode flags **without** `FILE_FLAG_FIRST_PIPE_INSTANCE`, as Windows requires for concurrent clients. Because Windows checks `FILE_CREATE_PIPE_INSTANCE` against the existing instance's DACL for every instance after the first, that right is granted to the invoking user and `LOCAL_SYSTEM` so the holder can create these instances; under §11.1's same-user trust boundary a same-user peer holding it is within the trust model, while `PIPE_REJECT_REMOTE_CLIENTS` and the DACL still exclude every remote and other-user principal. The initial secured instance and all holder state needed for the identity exchange exist before the marker is renamed into place.
...
The named pipe is byte mode, overlapped, local-only (`PIPE_REJECT_REMOTE_CLIENTS`) and protected by a non-inheriting DACL granting only the invoking user and `LOCAL_SYSTEM` the individual read/write/synchronise rights needed to connect, plus `FILE_CREATE_PIPE_INSTANCE`, which Windows requires of the holder for every listening instance after the first (the concurrent-clients rule above). The exact access mask is `0x12019F`. Broader `FILE_GENERIC_WRITE` is not granted.
...
Native Windows support starts at Windows 10 version 1809 / Windows Server 2019 and uses ConPTY. **The input-fidelity promise has a higher floor.** On hosts whose ConPTY predates the public win32-input-mode request — builds before Windows 10 version 2004 / Windows Server 2022 — the platform translation layer cannot be asked to deliver the lossless win32 input carrier: application-cursor sequences and some resize-adjacent input bytes are dropped by the operating system itself, not by Moor, and this was demonstrated on the shipped binary rather than assumed. Such hosts remain supported for session lifecycle, attach/detach, output transparency, close/termination, storage, and instrumentation; lossless viewer input is promised only where the win32-input-mode request is honored **and natively proven under §12.8** — today Windows Server 2022 and Windows 11. A Windows 10 host from version 2004 carries the same console layer and is expected to behave identically, but it joins the promise only through its own §12.8 lane, never by extrapolation. A viewer on a below-floor host attaches and operates, and input degrades exactly as it does in that host's console without Moor in the path.

The holder creates a pseudoconsole at the selected geometry and starts a small in-console bootstrap suspended with an explicit handle list. It creates a new no-breakaway job with kill-on-close, assigns the suspended bootstrap to that job, establishes the private bootstrap control/identity channel, and only then resumes the bootstrap. If job assignment or nested-job compatibility fails, launch fails before publication; the requested child is never run outside the promised containment as a fallback.
...
**Viewer-side input-mode negotiation.** On a host at or above the input-fidelity floor, the attaching viewer enables win32-input-mode on **its own console input** by writing exactly the bytes `1B 5B 3F 39 30 30 31 68` (`ESC[?9001h`) at attach and restores the prior state with `1B 5B 3F 39 30 30 31 6C` (`ESC[?9001l`) on detach, including abnormal detach paths. This is a negotiation between the viewer and its own terminal, not a holder preamble: the bytes are written to the viewer's console handle, never enter the child's stream, never cross a session transport, and are not logged. The win32-to-VT input translation this requests is performed by the operating system's console layer, so Moor adds no terminal emulator. What the viewer does recognize is exactly one thing: §6.2's canonical win32 keyboard-carrier form, solely to find the configured detach byte — the same narrowly scoped detach-key detection §1.1 already names, extended to the encoding the negotiated mode makes the console speak. There is no general CSI parsing, no resize parsing, and no output-side parsing. The controls are attempted on every supported Windows host: below the input-fidelity floor the platform may ignore or decline the request, and lossless input is simply not promised there. The below-floor evidence in §12.8 was produced with the controls attempted, so no OS-version gate exists in the viewer and none is claimed here.

On POSIX, a signal handler only records an atomic flag and wakes the normal event loop; it never calls `exit`, allocates, logs, closes descriptors or runs cleanup. Windows `CTRL_C_EVENT`/`CTRL_BREAK_EVENT` callbacks follow the same record-and-wake rule. The first accepted termination notification setting that flag starts one ten-second monotonic whole-shutdown deadline. The normal wake path abandons peer waits and never waits for a store worker. Graceful child termination begins immediately, escalates at five seconds, and at ten seconds the holder closes remaining handles and exits, retaining rendezvous evidence whenever lifecycle durability or child termination is uncertain. A second notification escalates immediately but never resets either deadline. No peer response, flush, callback, diagnostic, or thread join may extend ten seconds. OB-42 conformance drives these paths through the shipped binary rather than a handler unit test.

**The Windows terminal-control class runs the same sequence on a compressed clock.** For `CTRL_CLOSE_EVENT`, `CTRL_LOGOFF_EVENT`, and `CTRL_SHUTDOWN_EVENT` the operating system terminates the process unconditionally soon after the callback returns — the close budget is roughly five seconds — so the universal schedule cannot fit inside it. For exactly this class the callback records the same flag and then deliberately remains blocked, because returning is what authorises the platform kill; it still performs no cleanup, allocation, or logging of its own. Graceful child termination begins immediately, force escalation occurs at **two seconds**, and durable retirement of rendezvous and lifecycle evidence completes before the platform's terminal-control deadline, closing remaining handles as the final step. A second notification still escalates immediately, and nothing may extend the platform budget. All other notifications — POSIX signals, `CTRL_C_EVENT`, `CTRL_BREAK_EVENT` — keep the five-second escalation and ten-second whole-shutdown schedule above.
...
- **Windows:** ordinary `CreateProcess` descendants inherit job membership while WMI/broker-created or explicitly breakaway processes may not. No breakaway flag is granted by Moor. Graceful termination asks the in-console bootstrap to send `CTRL_BREAK_EVENT` to the requested child's process group and waits 5 seconds — or 2 seconds when the shutdown was started by §12.3's terminal-control class, whose compressed clock governs the whole sequence. Force, escalation, bootstrap loss, or `-f` calls `TerminateJobObject` with exit code `0xC000013A` (`STATUS_CONTROL_C_EXIT`); that exact unsigned DWORD is preserved in the exit event and foreground `run` result. Only processes actually in the job are guaranteed to end; Moor never claims causal descendants outside it were reached.
...
POSIX child status is 0–255 or a signal. Windows `GetExitCodeProcess` supplies an unsigned 32-bit code. Event schema v2 and the durable exit record preserve all 32 bits; foreground `run` exits through `ExitProcess(code)` with the same DWORD. A shell that chooses to display that DWORD as signed does not change the wire value. When Moor itself terminated the Windows job, the record's mandatory `method` field carries `"graceful"` or `"forced"` while `ended` stays `"exited"` with the code; an external termination records `method:"none"` and remains indistinguishable from an ordinary exit by its code alone — the method axis, not a folded ending, is what separates the two.
...
| native Windows | Windows Server 2022 x64 and Windows 11 arm64 at the input-fidelity floor, each with VT-native and legacy Console API children and the win32 input carrier; Windows 10 1809 x64 and Windows Server 2019 x64 as below-input-floor lanes exercising lifecycle, attach/detach, output transparency, close/termination, storage, and instrumentation, with the input-carrier cases recorded as expected-absent rather than waived silently |

Every lane exercises create/attach/detach/input/replay/termination, lease acquire/release/resume/loss, query arbitration, live and stale clear, same-user and wrong-user peers, generation/incarnation fencing, `-S` staging plus wrong-architecture failure, event/log/lifecycle recovery, every dual-slot crash prefix, scanner degradation, canonical JSON/delivery-control vectors, byte-exact CLI vectors, and real supervisor restart while the holder lives. The native-Windows lanes additionally exercise hostile inherited ACLs, marker/root/slot reparse points, remote-pipe refusal, impersonation failure, nested-job refusal, graceful CTRL_BREAK and forced job termination, UTF-16 names including an unpaired surrogate, full u32 exit codes, and x64/arm64 instrumentation. A missing lane narrows the release's stated support; it is not converted into a paper waiver.

The supervised-Windows launch lane also proves both exact source forms and forwarding into the `windows-spawn` child table; case-insensitive selector removal; adapter/native mutual exclusion; all short, long, missing-EOF, malformed, timeout, and I/O outcomes for the launch record and private receipt; preparation failure at every mutation with either proved exact rollback or creator-local `PreparationIndeterminate`; and crash boundaries before reservation creation, during its create/write/file-flush/parent-flush, and after every later prepared object. It proves that any reservation prefix refuses same- and later-generation automatic preparation, that the resolved absolute event identity is the one authenticated and persisted, that preexisting directories and path successors survive, and that the requested child inherits no launch, adoption, preparation, writer, publication, deletion, or receipt handle or selector.

Native process evidence proves atomic assignment to the creator-owned kill-on-close launch Job, destruction of every creator writer/publication handle and the complete suspended command before resume, checked never-resumed abort plus exact terminate-and-wait, and indeterminate handling for `ResumeThread` failure, an unexpected suspend count, or uncertain death. Ordinary destructor behavior is not rollback evidence. A vendor-crate unit helper covers the otherwise unobservable interval after successful `CreateProcessW`/atomic Job assignment and before return; real-Moor hard-death tests cover before the vendor call, immediately after the infallible armed-wrapper construction, and every later suspended/running pre-receipt checkpoint, without claiming a real-Moor hook between vendor return and that wrapper move. The exact capability inventory is inspected while suspended. MORR tests prove state-1-before-ACK-before-ready ordering, checked complete writes with poisoning after a short/error write, complete-first-state-3 plus exact-death rollback, and zero-byte EOF, timeout, partial/invalid/read-error, no-state-3, and post-state-1 deletion refusal.
...
Runtime branches are exact. `<error>` is the platform's nonempty single-line error text only on the legacy child-exec row. On the path/sink rejection rows, `<cause>` is exactly one of `missing`, `wrong-type`, `not-directory`, `not-searchable`, `link`, `reparse-point`, `wrong-owner`, `wrong-mode`, `broad-dacl`, `not-empty`, `extra-entry`, `pre-existing-slot`, `not-absolute`, `outside-root`, `identity-changed`, `io-error`, `wrong-architecture`, or `load-unacknowledged`. `not-absolute` is legal only on the event-target and instrumentation-rejection rows; in either row `<path>` preserves the original operand spelling under OB-29 rendering, as §§4.7 and 8.1 require.

On the supervised-launch row, `<cause>` instead uses this separate closed vocabulary: `generation-missing`, `generation-malformed`, `generation-disagree`, `selector-invalid`, `channel-timeout`, `record-wrong-length`, `record-malformed`, `generation-mismatch`, `preparation-failed`, or `io-error`. Validation proceeds through source selection, channel completion, record shape, generation carriers, record/carrier agreement, then any deferred preparation outcome; the first failing stage supplies the cause. `selector-invalid` covers adapter plus native selector, an adapter value other than exact `stdin-v1`, the adapter on anything except background `start`, missing/invalid/non-pipe standard input for that adapter, a present noncanonical native selector, a handle outside the explicit inheritance list, or the wrong native-handle type. Windows environment lookup and removal are case-insensitive. System failure while privately duplicating or forwarding a validated source is `io-error`.

`record-wrong-length` means either clean EOF before 32 bytes or observation of any 33rd byte, which rejects an overlong record immediately without waiting for EOF. `channel-timeout` means the record-plus-EOF condition remains absent or incomplete at the two-second deadline without either of those definitive length outcomes; an exact 32 bytes without EOF therefore times out. An exact-length record with bad magic, format, reserved bytes, or out-of-range record generation is `record-malformed`; another channel-system failure is `io-error`. Once selector, channel, and record validation succeeds, carrier precedence is presence, then grammar/range, then pair equality, then record equality: either carrier absent is `generation-missing` even if the present one is malformed; with both present, either bad grammar or range is `generation-malformed`; two valid unequal values are `generation-disagree`; and a valid equal pair different from the record is `generation-mismatch`.

A controlled preparation failure, including a reservation conflict, is deferred until the holder has successfully validated the record and carriers; it then reports exact `preparation-failed`, exits 1, emits MORR state 3 with result code 1, and creates no requested child or marker. Its `failed-v1` envelope contains no filesystem or adoption capability. Creator-local `PreparationIndeterminate` is the sole exception to holder-side record/carrier precedence: when an already mutated preparation cannot be rolled back with exact identity-fenced proof, the creator constructs no envelope, spawns no holder, closes every live capability, reports `io-error`, and preserves the reservation and debris for explicit whole-transaction cleanup. A wholly absent adapter and native selector remains the unsupervised branch and is not a rejection.
...
| **OB-1** | Reserve `.log`, `.events`, `.exit`, and `.instrument` under the platform comparison; reject every bare/path final component that collides or uses a Windows alias form; use the already reserved `.instrument` ending for the canonical `.exit.instrument` preparation reservation |
...
12. the event sink becoming a portable four-slot committed directory on every platform, preserving absolute operands while adding only the closed Windows root-relative leaf before mutation, with optional validated-empty creator provisioning and live-clear RPC;
...
The holder writes the derived `_SESSION_V2` value frozen in §4.4.1: `v2:` plus colon-separated canonical padded-base64 encodings of the absolute native rendezvous paths; POSIX encodes native bytes and Windows encodes canonical round-tripping WTF-8. Revision 4 removed the colon-joined legacy carrier entirely — it is neither written nor read, and a contradictory legacy value in the environment changes nothing. `current` and the supervisor read V2 and nothing else. This is the only option that handles Windows drive paths and arbitrary parent/base-name paths without delimiter ambiguity.

#### OB-1 — reserved suffix grammar

The reserved suffixes are exactly **`.log`**, **`.events`**, **`.exit`**, and **`.instrument`**. §2.1 owns the only path mapping: `.log` and `.exit` are appended to the rendezvous final component; a POSIX event operand and every absolute event operand are used exactly as supplied, while the closed Windows leaf is resolved under the invoked-binary Moor root; and the immutable stage remains `<H>.instrument` in the owner-only root. The preparation reservation is `companion(marker, ".exit.instrument")`: its existing `.instrument` ending is already unnameable and discovery-hidden, while the embedded non-hex `.exit` makes it disjoint from an exact 64-lowercase-hex stage prefix. The restriction applies to the final native path component of **both bare and path-form session names**; reservation of `.events` does not otherwise derive or rewrite an event path.
...
| **OB-10** | POSIX and every absolute `-T` operand name the exact supplied native path. Windows additionally accepts only §8.1's one-component lexical leaf, resolves it under the invoked-binary Moor root, preserves the original spelling for diagnostics, and uses the absolute target for every security/durable surface. The portable four-slot target may be absent for exclusive creation or be the exact validated empty caller-created directory; slots are always Moor-created exclusively and no predecessor state is adopted |
...
| **OB-16** | The actual holder consumes a **private inherited descriptor/HANDLE** selected by `<BASENAME>_LAUNCH_CHANNEL`; the exact 32-byte record plus EOF, 2-second deadline, generation check, and stripping rules are frozen in §10.1.1 and wire §15.1. On Windows the first process may transport the source through exact `BASE_LAUNCH_ADAPTER=stdin-v1`, but removes that adapter and forwards a private duplicate into the detached holder's explicit child handle table. The adapter, preparation envelope, selector, or carrier alone never establishes supervision; only the holder's valid launch record does, and no private transport reaches the requested child |
...
| **OB-19** | Both dimensions zero preserve; exactly one zero is malformed. Each real dimension is `1..32767`, widened product at most `2,000,000`, with checked Windows conversion (§4.3, §10.2.8) |
...
| **OB-42** | One ten-second monotonic deadline starts at the first accepted termination flag. Normal wake abandons peer/store waits, graceful termination starts immediately, five seconds escalates, and ten seconds closes handles/exits while retaining uncertain evidence. §12.3's Windows terminal-control class (`CTRL_CLOSE`/`CTRL_LOGOFF`/`CTRL_SHUTDOWN`) runs the identical sequence on its compressed clock: escalation at two seconds and durable retirement before the platform's ~five-second kill. A second notification escalates but resets nothing; no response, flush, callback, diagnostic, or join extends either deadline (§12.3) |
