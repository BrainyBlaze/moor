...
**Status:** authoritative amended implementation handoff. **Every section is written and normative.** §14 records the resolution of all 42 obligations and names the sole downstream runtime gate that remains open with its owner. Wire schema 5 and event schema 2 accompany this document. Revision 5 adds the explicit `LIVE_ONLY` attach replay policy and lease-authorized `REDRAW` request while preserving revision 4's status-first prefix and every legacy-surface retirement. The three authoritative inputs are named in §0; `spec/README.md` publishes the two normative document digests, for `moor-spec.md` and `moor-wire-schema.md`, that identify the amended handoff. Any later frozen-layout change requires a version increment.
...
**A second cutover obligation is recorded here for the same reason (OB-43).** A holder keeps running across supervisor restarts (OB-26), so a controller whose frozen wire version differs from a retained live holder's is not a cutover but the mixed-version window this section forbids. Before activating a supervisor or controller, the supervisor MUST, while the current compatible controller remains authoritative, inventory the retained live topology and each retained holder's **authenticated live** wire version — the version the live holder itself presents on its identity exchange under the generation the supervisor allocated, never a persisted record read in its place. A stored `(generation, version)` pair is at most a hint: a holder respawned or superseded since it was written makes the record stale, and a preflight that trusts it either permits a mixed-version window or refuses a compatible one, so the record MUST NOT decide. If the candidate controller's frozen wire version is incompatible with any retained live holder, activation MUST abort without signalling, retiring, or otherwise mutating the current controller or any holder, and the refusal MUST be a typed terminal outcome distinguishable from a generic attach failure, so that an operator can recognise it across the repository boundary. Proceeding requires explicit operator authorisation for a coordinated retirement and cutover of those holders; nothing infers it. A holder answers a foreign wire version with the companion schema's `UNKNOWN_VERSION` error (its §11) and closes only that connection; that response is terminal and non-retryable, and it MUST NOT enter an adoption retry loop, because no retry can change either peer's frozen dialect. Like the binary-resolution order above, this belongs to the cutover's definition of done rather than to the holder's own behaviour: the holder has no v3 decoder, no downgrade path, and no mixed dialect, and this clause adds none.
...
What MUST fail, loudly, is a *final component* that cannot fit: the program says so rather than silently truncating and operating on a different session than the caller named. The bound is the platform's local socket address capacity — `sun_path` less its terminator, 107 bytes on Linux and 103 on macOS — because the final component is addressed bare from its verified parent on every platform; the refusal is §13.3's rendezvous-rejection row with cause `too-long`, raised before any allocation, companion, or publication. Resolving the directory MUST NOT rely on changing the process-wide working directory, which races with anything else in the process.
...
Whenever the size changes — because a viewer is granted the input lease with a different requested size, or because the lease holder's terminal is resized — the child MUST be told, and an ordinary `RESIZE` MUST otherwise have no native effect. Repaint is a distinct `REDRAW` operation (§6.3): it applies the validated geometry and emits one native resize notification even when that geometry equals the current value. An observer that does not hold the lease can request neither operation (§6.1).
...
- Generation fencing, event-stream commit recovery, and both attach replay policies survive a real restart: create a session, attach, **restart the supervisor while the holder keeps running**, and observe that adoption re-establishes identity, reports replay exactness honestly, refuses a superseded generation, and either delivers the frozen retained baseline or adopts its live-only frontier exactly as requested. Wire v5 has no screen checkpoint (§6.7). Restarting the *holder* is not required and is not achievable — it owns the pseudo-terminal and no successor can reopen it by name (OB-26).
...
`ATTACH` is the only operation that creates a viewer. A connection becomes **fully attached** atomically when the holder freezes its replay descriptor and changes its phase before queuing the preamble; status and `list` count it from that instant. An input-only `push` connection and a probe never count as viewers. The attach flags select one of two named replay policies. With `LIVE_ONLY` clear, the retained policy sends the status-first sequence `ATTACH_ACK`, terminal-state preamble, `LEASE_RESULT` when requested, frozen replay, then live output. With `LIVE_ONLY` set, the same prefix is sent but the frozen descriptor's last record and exclusive byte end become the controller's starting frontier: no retained `GAP` or `OUTPUT` is sent, and only later live output may follow. Bits above `LIVE_ONLY` are refused, never ignored. The requested attach geometry is applied to the native terminal BEFORE the descriptor is built; a native refusal fails the attach closed — the holder closes the link. That failing attach is one atomic transaction FOR THE LEASE, committing on the successful enqueue of the token-bearing lease result: any earlier failure — deadline, native resize, or any prefix frame that cannot enqueue, the token frame itself included — rolls a fresh grant back entirely, reservation and epoch allocation both. The token never left the holder, so the epoch was never consumed and the next fresh controller receives that very number; a resumed viewer's failure instead preserves its known epoch/token reservation exactly as ordinary link loss does. The native geometry is the honest exception to that unwinding: once the native resize has succeeded, a LATER prefix failure may leave the applied geometry standing — a native effect may already be visible and is never guessed back with a compensating resize — and the next status descriptor simply reports the geometry actually in force. A `NON_VT` viewer may own input and geometry but receives an empty preamble, is never a query delegate, and receives none of the viewer-generated controls below; raw child output remains unchanged.
...
`-r` selects how the child is prompted to repaint: `none` sends nothing; `ctrl_l` sends exactly byte `0C`; `winch` sends one `REDRAW` after attach. `REDRAW` is not a disguised same-size `RESIZE`: it is the explicit lease-authorized repaint operation defined in §10.2.13.
...
Redraw occurs only if the connection owns the lease after attach; a busy observer never prompts the child. The reference CLI requests retained replay. It waits for the holder's non-interleaved ACK/terminal-state/optional-lease-result prefix and complete frozen replay, then performs the selected `ctrl_l` input or `REDRAW` request. A live-only controller may send `REDRAW` immediately after adopting the attach frontier; repaint output is ordinary live output and no retained baseline is introduced.
...
- On every successful attach, the holder freezes the retained first/last descriptor placed in `ATTACH_ACK`. A retained attach sends a `GAP` for `1..first-1` when a prefix was discarded, then sends every retained `OUTPUT` record in sequence before later live records on that connection. Output arriving during the baseline is ordered behind the frozen baseline. A controller that already consumed some records discards duplicates by record sequence; a fresh emulator applies the whole retained run. A live-only attach sends no retained `GAP` or `OUTPUT`; its controller adopts the descriptor's `last,end` frontier, and the first later live record must begin at `last + 1,end`.
- Empty history has no `GAP` and no `OUTPUT`. Its descriptor is the canonical half-open sequence interval `first=next_sequence,last=next_sequence-1` with `start=end=next_output_offset`; the initial empty stream is therefore `1,0,0,0`. This carries only the next live frontier and retains no bytes. A retained run beginning at record 1/byte 0 is complete raw history for this holder incarnation. Any later start is explicitly degraded: an exact modes preamble can restore the tracked modes, but Moor does not claim that a suffix reconstructs the screen. If tracked-mode exactness was lost, the mandatory preamble is empty rather than asserting guessed state.
...
Writers never mutate bytes inside the selected prefix. Growth removes any older uncommitted tail, writes from the selected prefix length, flushes the body, writes the alternate commit at offset zero, truncates it to 92 bytes, and flushes it. Replacement writes the inactive body from offset zero, truncates and flushes it, then writes and flushes the alternate commit pointing to it. File creation and removal additionally flush the containing directory. A failure before a new valid commit is selected leaves the prior commit authoritative. An I/O failure during commit mutation or flush is explicitly ambiguous: either the old or submitted commit may later validate, so no rollback is guessed and that store closes permanently. A missed caller deadline closes the lane immediately, but once alternate-commit mutation has started it does not cancel that synchronous transaction between syscalls: the quarantined worker may finish exactly that candidate through its commit flush and publish the selected frontier, while no later transaction may begin (§8.5).
...
Initial store commits run in parallel under one two-second launch gate. At runtime the oldest item in each lane must obtain a selected durable commit or close that lane to its caller within two seconds of admission. Queue overflow on a mandatory holder observation, an I/O error, or a missed progress deadline atomically closes only that store, clears later queued work, and quarantines its worker. A semantic request that would cross the event bound is instead refused before state change with `SEM_RESOURCE_EXHAUSTED`. The main PTY loop never joins or waits for a stuck worker. Quarantine before alternate-commit mutation prevents that commit from starting. Once alternate-commit mutation has started, cancellation between its synchronous syscalls would create a permanently unknown frontier, so the worker may finish exactly that candidate through its commit flush and publish the selected frontier; the lane remains closed and no later transaction may begin. Readers may therefore select either the previous or that one submitted candidate, never a later one.
...
- the **retained-history descriptor**: the first/last retained output record sequence, the half-open retained byte-offset range, whether that raw history begins at byte zero, and whether the tracked terminal-mode state is exact or degraded (§6.7). A live-only attach uses its last/end pair as the starting frontier without receiving retained frames. There is no screen checkpoint or main/alternate-buffer exactness field in wire v5.
...
- It is sent **exactly once per attaching connection**, and **immediately after** the attach acknowledgement — the status-first order retained by revision 5. A probe that never sends `ATTACH` receives none. A second preamble, a missing preamble on attach, or one arriving before its acknowledgement is a protocol error.
...
| `1B` | `REDRAW` | controller to holder | u32 current lease epoch, then geometry (§4.3) |

Payload lengths are respectively 40, 24, 20, 20, 24, 32, and 8 bytes. `MORE` is forbidden; any other length or fragmentation is `MALFORMED_FRAME`.
...
| `V` attached lease viewer | yes | yes | `INPUT`, `RESIZE`, `REDRAW`, `QUERY_REPLY`, `LEASE_KEEPALIVE`, `LEASE_RELEASE`, `LOG_CLEAR` |
...
Controller wire v5, semantic wire v1, JSON event schema v2, the four-slot event/log/lifecycle store, checksums, integer byte order, generations, correlations and provenance are portable between the two families. Native path values are raw POSIX bytes on every surface.
...
Runtime branches are exact. `<error>` is the platform's nonempty single-line error text only on the legacy child-exec row. On the path/sink rejection rows, `<cause>` is exactly one of `missing`, `wrong-type`, `not-directory`, `not-searchable`, `link`, `wrong-owner`, `wrong-mode`, `not-empty`, `extra-entry`, `pre-existing-slot`, `not-absolute`, `outside-root`, `identity-changed`, `io-error`, `wrong-architecture`, `load-unacknowledged`, or `too-long`. `too-long` is legal only on the rendezvous-rejection row, whose `<path>` is the resolved absolute rendezvous path whose final component exceeds §2.2's address capacity. `not-absolute` is legal only on the event-target and instrumentation-rejection rows; in either row `<path>` preserves the original operand spelling under OB-29 rendering, as §§4.7 and 8.1 require.
...
| rendezvous rejected | `rendezvous rejected: <path> (<cause>)` | stderr / LF / 1 |
...
| **OB-36** | `INPUT_RECEIPT` remains exactly transport-written/refused. Wire v5 optionally binds the input to an application request id and semantic source, and requires a prepared `INPUT_NOTICE` before the PTY write. That enables, but does not fabricate, OB-37 evidence |
...
| **OB-39** | `ATTACH_ACK` and `STATUS_REPLY` carry §10.2.5's complete descriptor: identity/incarnation, portable event frontier, starts/boot/cwd/child, replay/tracked exactness, fully-attached/child/event flags, store/observer/query health, and selected log epoch/index/range. Wire v5 still claims no screen checkpoint |
...
| **OB-43** | Supervisor activation across a wire-version change is a coordinated cutover, never a mixed-version window: before activating a controller the supervisor inventories retained live holders and each one's authenticated live wire version while the current compatible controller stays authoritative — a persisted `(generation, version)` record never decides — aborts activation without mutating any controller or holder when the candidate is incompatible with any live holder, surfaces a typed terminal refusal distinguishable from a generic attach failure, and proceeds only on explicit operator authorisation for coordinated retirement. `UNKNOWN_VERSION` is terminal and non-retryable and never enters an adoption retry loop (§0.2) |

### 14.3 Still open — one, with its owner

| id | why it cannot be closed here | owner |
|---|---|---|
| **OB-37** | Wire v5 and §10.3 now provide the application-correlation carrier, provenance, durable event and loss/recovery behavior. The remaining gate is **real provider authority**: Moor cannot make a provider read or act, and a frame named `APPLICATION_RECEIPT` is not proof that any given provider can truthfully emit it. Closure is therefore per provider/version only after shipped end-to-end runtime conformance at the named authoritative point | **The supervisor specification jointly with each provider integration.** Until the deployed provider passes that gate, the receipt path is not authoritative for that provider. No global closure follows from this document |
