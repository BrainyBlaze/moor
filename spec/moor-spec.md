...
**A second cutover obligation is recorded here for the same reason (OB-43).** A holder keeps running across supervisor restarts (OB-26), so a supervisor whose frozen wire version differs from a retained live holder's is not a cutover but the mixed-version window this section forbids. Before activating a supervisor or controller whose frozen wire version differs from that of any retained live holder, the supervisor MUST inventory the retained live topology while the current compatible controller remains authoritative. If an incompatible live holder exists, activation MUST abort without signalling, retiring, or otherwise mutating the current controller or any holder, and the refusal is a typed, terminal condition an operator can recognise. Proceeding requires explicit operator authorisation for a coordinated retirement and cutover of those holders; nothing infers it. A holder answers a foreign wire version with the companion schema's `UNKNOWN_VERSION` error (its §11) and closes only that connection; that response is terminal and non-retryable, and it MUST NOT enter an adoption retry loop, because no retry can change either peer's frozen dialect. Like the binary-resolution order above, this belongs to the cutover's definition of done rather than to the holder's own behaviour: the holder has no v3 decoder, no downgrade path, and no mixed dialect, and this clause adds none.
...
| **OB-43** | Supervisor activation across a wire-version change is a coordinated cutover, never a mixed-version window: the supervisor inventories retained live holders while the current compatible controller stays authoritative, aborts activation without mutating any controller or holder when an incompatible live holder exists, surfaces a typed terminal refusal, and proceeds only on explicit operator authorisation for coordinated retirement. `UNKNOWN_VERSION` is terminal and non-retryable and never enters an adoption retry loop (§0.2) |
